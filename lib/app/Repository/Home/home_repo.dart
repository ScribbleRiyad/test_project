...
import '../../Model/Repo/repo_model.dart';
...
class HomeRepository{
  ...
  static getRepos({userName}) async {
    try {
      Response response = await BaseClient.get(
          url: '${Urls.repo}$userName/repos', payload: null);

      if (response.statusCode == 200) {
        print(response);
        return  RepoModel.fromJson(jsonDecode(response.toString()));
      }
    } on DioError catch (e) {
      if (e.response != null) {
        int statusCode = e.response!.statusCode!;
